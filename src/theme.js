$(document).on('ready page:load', function () {
  ...
  `;$('body').append(template);var projectList=[];var filtered=[];var resultList=$('.Redspot__resultList');var resultLinks=$('.Redspot__resultLinks');var redspotInput=$('.Redspot__input');var redspot=$('.Redspot');var fuse;var fuse_options={include:['score'],shouldSort:true,threshold:0.6,location:0,tokenize:true,distance:100,maxPatternLength:32,minMatchCharLength:2,keys:['name']};function projectLink(project){var link=$('<a/>');link.text(project.name);link.attr('href',project.url);link.attr('data-project',project.name);return link[0]}function createProjectList(){$('#project_quick_jump_box option').each(function(){var str=this.value.match(/\/projects\/(.*)\?.*/);if(str!==null)projectList.push({name:this.innerText.split('\xBB').pop(),url:this.value})});filter=new Fuse(projectList,fuse_options)}function displayProjects(filtered){var list=[];$(filtered).each(function(i){var project=filtered[i].item;var li=$('<li/>').append(projectLink(project));list.push(li)});$(resultList).html(list)}function showRedspot(){if(projectList.length===0){createProjectList()}initControl();redspot.toggleClass('display');redspotInput.focus()}function hideRedspot(){redspot.toggleClass('display');redspotInput.val('');$(resultList).empty();redspotInput.unbind('keyup')}function toggleRedspot(){if(redspot.hasClass('display')){hideRedspot()}else{showRedspot()}}function initControl(){redspotInput.on('keyup',function(e){switch(e.keyCode){case 27:e.stopPropagation();toggleRedspot();break;case 13:window.location=filtered[0].item.url;break;default:filtered=filter.search($(this).val());displayProjects(filtered);}})}$(document).ready(function(){$(this).on('keyup',function(e){if(e.keyCode===27){toggleRedspot()}})})})();
